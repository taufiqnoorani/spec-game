...
def getScorecards():
    scorecardList = []
    scorecards = pd.read_csv("specAI/scorecards.csv")
...
def getShowcards():
    ...
    showcards = pd.read_csv("specAI/showcards.csv")
    i = 1
    for r in randomList:
        sc = [str(i), str(showcards.iloc[r].Rank),
              str(showcards.iloc[r].Suite), "0"]
...
def generateGrid(showcards):
    ...
    for i in randomList:
        ...
        if (column == 5):
            column = 0
            grid = pd.concat([grid, pd.DataFrame([rowList])],
                             ignore_index=True)
            rowList.clear()

    # print(grid)
    grid.to_csv("specAI/grid.csv")


def populatePlayerScoreDict(players):
    playerScores = {}
    for x in players:
        playerScores[x] = []
    return playerScores
